...
def _spawn_env():
    """子程序需要 PYTHONPATH 才能找到 video_notes 套件"""
    env = os.environ.copy()
    ...
    def do_POST(self):
        if self.path == '/api/save':
            try:
                ...
                yaml_path = DATA_DIR / video_id / "data.yaml"
                ...
                # Re-render
                subprocess.run([sys.executable, "-m", "video_notes", "--render-only", str(yaml_path)], cwd=PROJECT_ROOT, env=_spawn_env(), capture_output=True)
            ...
            try:
                ...
                lib_path = DATA_DIR / "library.yaml"
                
                with open(lib_path, 'w', encoding='utf-8') as f:
                    yaml.dump(data, f, allow_unicode=True, sort_keys=False, default_flow_style=False)
                
                # Re-generate index.html after library change
                subprocess.run([sys.executable, "-m", "video_notes", "--index"], cwd=PROJECT_ROOT, env=_spawn_env(), capture_output=True)
